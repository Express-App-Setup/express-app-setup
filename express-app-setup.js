...
const resetColor = '\x1b[0m';
 ...
 const createConfigFolder = (projectName) =>{
   const projectPath = path.resolve(process.cwd(), projectName, "src", 'config');
   fs.mkdirSync(projectPath);
 }
 ...
 const createServerFile = (projectName, useTypeScript) => {
   const filePath = path.join(projectName, "src", useTypeScript ? 'server.ts' : 'server.js');
 ...
 const createCorsConfigFile = (projectName, useTypeScript)=>{
   const filePath = path.join(projectName, "src", 'config', useTypeScript? 'cors.ts' : 'cors.js');
 ...
 const createBodyParserConfigFile = (projectName, useTypeScript)=>{
   const filePath = path.join(projectName, "src", 'config', useTypeScript? 'body-parser.ts' : 'body-parser.js');
...
const createExpressApp = (projectName, useTypeScript) => {
  ...
  createAnyFolder(projectName, "src")
